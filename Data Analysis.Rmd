...
The engineering data is in 6 different csv files and must be combined. These are expected to be the result of running `RetrivalScript1.6.py`. The data retrieved from each SQL table is commented within the code.
...
First, convert all Unix timestamps to dateTime (posix objects)

```{r Tidy:Engineering:dateTime, message=FALSE, warning=FALSE}
...
Second, round the time to every 5 minutes. 

```{r Tidy:Engineering:5min, message=FALSE, warning=FALSE}
...
Third, average all the values that rounded to that time into one row 

```{r Tidy:Engineering:average, message=FALSE, warning=FALSE}
...
Fourth, combine all the engingeering data into a single master dataFrame

```{r Tidy:Engineering:master, message=FALSE, warning=FALSE}
...
sensorMaster %>%  
ggplot( aes(x=(log(Cn2)), color=sensor)) + 
...
# try to mutate in the dome slit naively and see if it works
engineeringMaster %>% select(dateTime, slit) %>%
...
ggplot( aes(x=(log(Cn2)), color=sensor)) +
  geom_histogram(bins=500) +
  facet_wrap(~slit, scales="fixed")

#wy do we have na's in the slit data????
engineeringMaster %>% filter(is.na(slit))
#okay why do we have NA in the tcs
tcs5_df %>% filter(date(dateTime)==as.Date("2022-08-26"))
#so we don't actually have NA's, but it does go offline. 
...
ggplot(scrap, aes(x=(log(Cn2)), color=sensor)) + 
  geom_histogram()
